...
public abstract class BaseInfluxDbElectricRepository<T> {
    ...
    public BaseInfluxDbElectricRepository(InfluxDBClient influxDBClient, String quantityName, String bucketName, String organization) {
    ...
    public List<T> findAllBetweenDates(Instant startDate, Instant endDate, List<ElectricPhase> phases) {
        String query = String.format("""
                from(bucket: "%1$s")
                  |> range(start: %2$s, stop: %3$s)
                  |> filter(fn: (r) => r._measurement == "%4$s"%5$s)
                  |> sort(columns:["_time"])""", BUCKET_NAME, startDate, endDate, QUANTITY_NAME, generatePhaseFilter(phases));
    ...
    public List<T> getValueSince(Instant since, List<ElectricPhase> phases) {
        String query = String.format("""
                from(bucket: "%1$s")
                  ...
                  |> sort(columns:["_time"])""", BUCKET_NAME, since, QUANTITY_NAME, generatePhaseFilter(phases));
    ...
    public List<List<T>> getGroupedMinMaxMean(Instant startDate, Instant endDate, List<ElectricPhase> phases) {
        ...
        String subquery = String.format("""
                from(bucket: "%s")
                  ...
                  |> sort(columns:["_time"])""", BUCKET_NAME, startDate, endDate, QUANTITY_NAME, generatePhaseFilter(phases), windowDuration, "%s");
    ...
    public List<T> getLastNValues(List<ElectricPhase> phases, int count) {
        String query = String.format("""
                from(bucket: "%1$s")
                   |> range(start: 0)
                   |> filter(fn: (r) => r._measurement == "%2$s"%3$s)
                   ...
                   |> limit(n: %4$d)
                   |> sort(columns:["_time"])""", BUCKET_NAME, QUANTITY_NAME, generatePhaseFilter(phases), count);
